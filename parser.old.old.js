...
function humanizeFn(fn) {  
  let dirname = path.dirname(fn)
  let basename = path.basename(fn)
  let home = process.env.HOME
  let cwd = process.cwd()
  let cwd_fn = fn, home_fn = fn
  if (dirname.startsWith(cwd)) {  
    let dn = '.' + dirname.substring(cwd.length)
    cwd_fn = `${dn}/${basename}`
  }
  if (dirname.startsWith(home)) {  
    let dn = '~' + dirname.substring(home.length)
    home_fn = `${dn}/${basename}`
  }
  if (cwd_fn.length < fn.length) fn = cwd_fn
  if (home_fn.length < fn.length) fn = home_fn
  return fn
}
function getFile(fn) {  
...
function logCall(cmnt, deep) {  
  var caller = logCall.caller
  var args = [
  ].slice.call(caller.arguments, 0)
  cmnt = cmnt ? cmnt + ' ' : ''
  if (deep) {  
    log(cmnt + caller.name + '(')
    for (let arg of args) log(arg);
  } else {  
    log(cmnt + caller.name + '(' + args.join(', ') + ')')
  }
}
